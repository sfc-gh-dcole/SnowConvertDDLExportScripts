...
To extract the DDL for the tables in the database. 
For example, create a copy of PRODUCTION database called SNOWFLAKE such that all of the objects in the
first database are created in the new SNOWFLAKE database. 
We add -l to get user-defined spaces, database partition groups(dpg) and buffer pools to be produced as well.
